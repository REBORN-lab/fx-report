...
# 4(2026-08-14):**删掉 body_plan**。它的两个键在 `scripts/` 下都零读取 ——
#   `line` 从来没有被读过(唯一"读者"是 SKILL 里让 LLM 整句抄进正文的散文,
#   而那句「本日无可用增量,不更新判断」正属正文禁词那一类:执行它就违规,
#   不执行它就是死键);`mode` 在判断环那条 minimal 豁免删除后同样零读取。
#   一个没人读的脚本产出物是等着漂移的第三份拷贝,本轮病灶正是它。
#   「当日该币种无增量」这个事实没有丢:derived.events.<币种> 的结论句与
#   gaps 两条既有通道各自独立把它落到附录,_body_plan 只是第三遍算它。
#   删键**必须**升版本,理由与当初加它时逐字相同:不升会让"新代码产出的
#   快照"与"阶段 1 之前采的存量快照"在读者眼里同形。既有闸门
#   DERIVED_VERDICT_SCHEMA 是 `>= 2`,4 照样过,2 的含义(本该有结论句)
#   对 4 继续成立。
SCHEMA_VERSION = 4
RANGE_DAYS = 5
