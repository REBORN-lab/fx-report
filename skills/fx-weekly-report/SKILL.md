...
4. 读 `state/calendar-*.json`,摘出未来 2-3 周内(TODAY+1 … TODAY+21)的五央行
   议息与主要数据发布日程,供"下周关注"引用;年历过期或缺失时记入缺漏汇总。
...
只依据第 1 步素材写 `reports/weekly/WEEK.md`,模板:
    ...
    ## 复盘汇总
    >(图例:命中/未命中=触发发生且方向核对有果;无法判定=已复盘但触发条件
    > 未发生或证据不足;未判定=观点尚未到复盘窗口。本行不含数字)
    - <stats 命令输出的计数行,原样照抄>
