...
只读 `data/DATE.json`,写 `briefs/DATE-brief.md`,模板:
    ...
    - 昨日事件 top:<title>(<domain>,采见 <seendate>)……至多 3 条
      ...
      **`attributable_source_absent`**(脚本算好,三态,与 `articles` 一一对应):
      ...
      缺漏节如实陈述该币种事件采集失败。
      `source_capped` 只陈述一个条目级事实:落盘该条目的那条通道,本次返回的
      原始样本顶到了它自己的上限。**禁止**据它自行拼装任何关于条数多寡的话术
      ——事件面的判定一律以 `events_verdict` 为准(它读的是 `sample_capped`,
...
9. **事件结论句必须整句照抄进正文。** 要点表里 `events_verdict` 那一句,
   ...
   **例外(存量快照)**:要点表该行写的是"结论句不可得(存量快照)"时,
   根本没有句子可引,本条不适用——此时正文照常陈述该币种事件面的事实即可,
   并**禁止自行补造一句结论句**。校验器此时处于跳过档
   (`VERDICT_SKIPPED_NO_DERIVED` 或 `VERDICT_SKIPPED_LEGACY`,视快照缺的是整个
   derived 节还是只有 schema 过旧而定),编出来的句子一个字都抓不到,
   所以只能靠这一条挡住。
...
- `VERDICT_SKIPPED_NO_DERIVED`:快照没有 derived 节(derive 没跑,或整体失败),
  ...
  - **窗口已移过 DATE**(存量快照,如 `data/2026-08-07`~`data/2026-08-10`)→
    **无补救,照常结束**,处置与 `VERDICT_SKIPPED_LEGACY` 同:按第 2 步的
    "结论句不可得(存量快照)"处理,禁令 9 对该币种不适用,同样**禁止自行补造**。
...
- `VERDICT_SKIPPED_LEGACY`:derive 跑过,但 `derived.schema_version` 低于闸门
  ...
  没有结论句可引,按第 2 步的"结论句不可得(存量快照)"处理即可(禁令 9 对
