...
只读 `data/DATE.json`,写 `briefs/DATE-brief.md`,模板:
    ...
    - 派生指标:日涨跌 <chg_pct_1d>%,5 运行日区间 <range_5d_low>–<range_5d_high>
      ...
      **禁止**据 `count_capped` / `sample_capped` / `main_sample_capped` /
      ...
      不判定快照新旧**——属哪一支由第 5 步的判别标准决定:采集窗口仍覆盖 DATE
      时,先按第 5 步重跑第 1 步采集、再回本步重做;窗口已移过时才就此定稿。
...
9. **事件结论句必须整句照抄进正文。** 要点表里 `events_verdict` 那一句,
   ...
   **例外(快照未落结论句)**:要点表该行写的是"结论句不可得(快照未落结论句)"
   ...
   并**禁止自行补造一句结论句**。**适用范围以第 5 步的判别为准**:
   `VERDICT_SKIPPED_LEGACY` 一律适用;`VERDICT_SKIPPED_NO_DERIVED` **只在窗口
   已移过 DATE 那一支适用**——窗口仍覆盖 DATE 时那是当日 derive 崩了,该按第 5 步
   重跑采集把结论句补出来,不是免除逐字引用的理由。校验器此时处于跳过档,
   编出来的句子一个字都抓不到,所以只能靠这一条挡住。
...
- `VERDICT_SKIPPED_NO_DERIVED`:快照没有 derived 节(derive 没跑,或整体失败),
  ...
  - **窗口已移过 DATE**(存量快照,如 `data/2026-08-07`~`data/2026-08-10`)→
    **无补救,照常结束**,处置与 `VERDICT_SKIPPED_LEGACY` 同:按第 2 步的
    "结论句不可得(快照未落结论句)"处理,禁令 9 对该币种不适用,
    同样**禁止自行补造**。
    此时重跑采集拿回的不是当日那批条目,**不构成本码的补救手段**。
