...
主通道被相关性闸门滤除的条数 SHALL 参与跨日聚合的结论判定:整区间抓到大量条目却全部落在白名单外时,结论 MUST NOT 为「确实 0 条」,SHALL 退化为「未取得可署名来源、有无事件无法判定」并给出滤除条数。面向读者的结论句 MUST NOT 出现程序空值的字面量——区间内上限不唯一时改用不含数值的措辞。

主通道的截断标记 SHALL 记录在主通道自身的账内,补位通道覆写条目级字段时 MUST NOT 抹掉它。

「是否取得可署名来源的报道」SHALL 由采集层算成一个布尔落盘,报告层只引用该布尔,MUST NOT 由报告层自行组合「滤除计数为 0」与「文章列表为空」两个条件——补位成功的当日两个条件会给出相互矛盾的指引。
