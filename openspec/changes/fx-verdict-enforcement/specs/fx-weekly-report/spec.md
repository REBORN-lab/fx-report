...
未提供聚合文件时 SHALL 不执行结论句检查,MUST NOT 因取不到结论句而报「字段缺失」。此时校验器 SHALL 打印一行降级声明,明确本次未校验结论句与数字溯源;该声明 MUST NOT 计为违规、MUST NOT 改变退出码——「跳过」与「通过」在输出上 MUST 可区分,裸打印通过与「全部查过且全过」逐字不可分辨。

周报校验 MUST NOT 接受位置参数形式的快照路径;`--mode weekly` 下收到非空位置参数时 SHALL 以错误码 2 响亮失败并在标准错误给出可操作提示(聚合文件用 `--digest` 传入、当周日报用 `--daily` 传入)。校验器 MUST NOT 猜测该位置参数的意图、MUST NOT 将其当作聚合文件使用——猜测正是该缺陷的来源:调用方以为已传入、脚本以为未传入,两侧都不出声,于是结论句闸门与数字溯源整层不执行却打印通过。实测本条之前的形态:把该位置参数换成一个不存在的路径,校验仍打印通过并以 0 退出,证明该参数从未被读取。
...
#### Scenario: 未提供聚合文件时必须声明跳过
- **WHEN** 校验周报时未提供 `--digest`
- **THEN** 输出中出现降级声明 `WEEKLY_DIGEST_ABSENT_SKIPPED`,说明本次未校验结论句与数字溯源;该声明不计为违规、不改变退出码,且在结论行之前打印

#### Scenario: 提供了聚合文件时不得打印跳过声明
- **WHEN** 校验周报时提供了 `--digest`
- **THEN** 输出中 MUST NOT 出现该降级声明——无条件打印会使声明退化为噪声,「查过」与「没查过」重新不可分辨

#### Scenario: weekly 模式收到位置参数
- **WHEN** 以 `--mode weekly` 调用校验器,并在位置参数上给出一个路径
- **THEN** 校验以错误码 2 失败,标准错误指明改用 `--digest` 与 `--daily`,且 MUST NOT 打印任何结论行(打印警告后继续跑完并以 0 退出等同于未修复)

#### Scenario: 位置参数不得被当作聚合文件使用
- **WHEN** 以 `--mode weekly` 调用校验器,位置参数指向一份结构不符的文件,且未提供 `--digest`
- **THEN** 失败原因为「不接受位置参数」而非「聚合文件结构不符」——校验器 MUST NOT 猜测意图把它当聚合文件加载,即使同时提供了 `--digest` 也 SHALL 一律拒收
